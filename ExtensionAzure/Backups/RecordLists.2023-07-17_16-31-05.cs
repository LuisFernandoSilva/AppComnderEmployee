...
namespace OutSystems.NssExtensionAzure {
	...
	/// <summary>
	/// RecordList type <code>RLNewRecordList</code> that represents a record list of <code>New</code>
	/// </summary>
	[Serializable()]
	public partial class RLNewRecordList: GenericRecordList<RCNewRecord>, IEnumerable, IEnumerator, ISerializable {
		public static void EnsureInitialized() {}

		protected override RCNewRecord GetElementDefaultValue() {
			return new RCNewRecord("");
		}

		public T[] ToArray<T>(Func<RCNewRecord, T> converter) {
			return ToArray(this, converter);
		}

		public static T[] ToArray<T>(RLNewRecordList recordlist, Func<RCNewRecord, T> converter) {
			return InnerToArray(recordlist, converter);
		}
		public static implicit operator RLNewRecordList(RCNewRecord[] array) {
			RLNewRecordList result = new RLNewRecordList();
			result.InnerFromArray(array);
			return result;
		}

		public static RLNewRecordList ToList<T>(T[] array, Func <T, RCNewRecord> converter) {
			RLNewRecordList result = new RLNewRecordList();
			result.InnerFromArray(array, converter);
			return result;
		}

		public static RLNewRecordList FromRestList<T>(RestList<T> restList, Func <T, RCNewRecord> converter) {
			RLNewRecordList result = new RLNewRecordList();
			result.InnerFromRestList(restList, converter);
			return result;
		}
		...
		/// <param name="trans"> IDbTransaction Parameter</param>
		[Obsolete("Use the Default Constructor and set the Transaction afterwards.")]
		public RLNewRecordList(IDbTransaction trans): base(trans) {
		}
		...
		/// <param name="alternateReadDBMethod"> Alternate Read Method</param>
		[Obsolete("Use the Default Constructor and set the Transaction afterwards.")]
		public RLNewRecordList(IDbTransaction trans, ReadDBMethodDelegate alternateReadDBMethod): this(trans) {
			this.alternateReadDBMethod = alternateReadDBMethod;
		}
		...
		/// <param name="info"> SerializationInfo</param>
		/// <param name="context"> StreamingContext</param>
		public RLNewRecordList(SerializationInfo info, StreamingContext context): base(info, context) {
		}
		...
		/// </summary>
		/// <returns>The new record list</returns>
		protected override OSList<RCNewRecord> NewList() {
			return new RLNewRecordList();
		}
