...
namespace OutSystems.NssExtensionAzure {
	...
	/// <summary>
	/// Structure <code>RCtesteRecord</code>
	/// </summary>
	[Serializable()]
	public partial struct RCtesteRecord: ISerializable, ITypedRecord<RCtesteRecord> {
		internal static readonly GlobalObjectKey Idteste = GlobalObjectKey.Parse("2UmDmepsh0WSfJ_D1JexCA*55NGFnYllENQ3pF+dSVh6A");

		public static void EnsureInitialized() {}
		[System.Xml.Serialization.XmlElement("teste")]
		public ENtesteEntityRecord ssENteste;


		public static implicit operator ENtesteEntityRecord(RCtesteRecord r) {
			return r.ssENteste;
		}

		public static implicit operator RCtesteRecord(ENtesteEntityRecord r) {
			RCtesteRecord res = new RCtesteRecord(null);
			res.ssENteste = r;
			return res;
		}
		...
		public BitArray ChangedAttributes {
			set {
				ssENteste.ChangedAttributes = value;
			}
			get {
				return ssENteste.ChangedAttributes;
			}
		}
		public BitArray OptimizedAttributes;

		public RCtesteRecord(params string[] dummy) {
			OptimizedAttributes = null;
			ssENteste = new ENtesteEntityRecord(null);
		}
		...
		public BitArray[] AllOptimizedAttributes {
			set {
				if (value == null) {
					ssENteste.OptimizedAttributes = GetDefaultOptimizedValues()[0];
				} else {
					ssENteste.OptimizedAttributes = value[0];
				}
			}
			get {
				BitArray[] all = new BitArray[1];
				all[0] = ssENteste.OptimizedAttributes;
				return all;
			}
		}
		...
		/// </summary>
		/// <param name="r"> Record</param>
		public void ReadIM(RCtesteRecord r) {
			this = r;
		}


		public static bool operator == (RCtesteRecord a, RCtesteRecord b) {
			if (a.ssENteste != b.ssENteste) return false;
			return true;
		}
		...
		public override bool Equals(object o) {
			if (o.GetType() != typeof(RCtesteRecord)) return false;
			return (this == (RCtesteRecord) o);
		}

		public override int GetHashCode() {
			try {
				return base.GetHashCode()
				^ ssENteste.GetHashCode()
				;
			} catch {
				return base.GetHashCode();
			}
		...
		public RCtesteRecord(SerializationInfo info, StreamingContext context) {
			OptimizedAttributes = null;
			ssENteste = new ENtesteEntityRecord(null);
			Type objInfo = this.GetType();
			FieldInfo fieldInfo = null;
			fieldInfo = objInfo.GetField("ssENteste", BindingFlags.NonPublic | BindingFlags.Instance | BindingFlags.Public);
			if (fieldInfo == null) {
				throw new Exception("The field named 'ssENteste' was not found.");
			}
			if (fieldInfo.FieldType.IsSerializable) {
				ssENteste = (ENtesteEntityRecord) info.GetValue(fieldInfo.Name, fieldInfo.FieldType);
			}
		}

		public void RecursiveReset() {
			ssENteste.RecursiveReset();
		}

		public void InternalRecursiveSave() {
			ssENteste.InternalRecursiveSave();
		}


		public RCtesteRecord Duplicate() {
			RCtesteRecord t;
			t.ssENteste = (ENtesteEntityRecord) this.ssENteste.Duplicate();
			t.OptimizedAttributes = null;
			return t;
		}
		...
		public void ToXml(Object parent, System.Xml.XmlElement baseElem, String fieldName, int detailLevel) {
			...
			if (detailLevel > 0) {
				ssENteste.ToXml(this, recordElem, "teste", detailLevel - 1);
			} else {
				VarValue.AppendDeferredEvaluationElement(recordElem);
			}
		}
		...
		public void EvaluateFields(VarValue variable, Object parent, String baseName, String fields) {
			...
			variable.Found = false;
			if (head == "teste") {
				if (!VarValue.FieldIsOptimized(parent, baseName + ".teste")) variable.Value = ssENteste; else variable.Optimized = true;
				variable.SetFieldName("teste");
			}
			if (variable.Found && tail != null) variable.EvaluateFields(this, head, tail);
		}

		public bool ChangedAttributeGet(GlobalObjectKey key) {
			return ssENteste.ChangedAttributeGet(key);
		}

		public bool OptimizedAttributeGet(GlobalObjectKey key) {
			return ssENteste.OptimizedAttributeGet(key);
		}
		...
		public void FillFromOther(IRecord other) {
			if (other == null) return;
			ssENteste.FillFromOther((IRecord) other.AttributeGet(Idteste));
		}
		public bool IsDefault() {
			RCtesteRecord defaultStruct = new RCtesteRecord(null);
			if (this.ssENteste != defaultStruct.ssENteste) return false;
			return true;
		}
	} // RCtesteRecord
}
