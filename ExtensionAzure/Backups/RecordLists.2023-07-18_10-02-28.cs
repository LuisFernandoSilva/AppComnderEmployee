...
namespace OutSystems.NssExtensionAzure {
	...
	/// <summary>
	/// RecordList type <code>RLtesteRecordList</code> that represents a record list of <code>teste</code>
	/// </summary>
	[Serializable()]
	public partial class RLtesteRecordList: GenericRecordList<RCtesteRecord>, IEnumerable, IEnumerator, ISerializable {
		public static void EnsureInitialized() {}

		protected override RCtesteRecord GetElementDefaultValue() {
			return new RCtesteRecord("");
		}

		public T[] ToArray<T>(Func<RCtesteRecord, T> converter) {
			return ToArray(this, converter);
		}

		public static T[] ToArray<T>(RLtesteRecordList recordlist, Func<RCtesteRecord, T> converter) {
			return InnerToArray(recordlist, converter);
		}
		public static implicit operator RLtesteRecordList(RCtesteRecord[] array) {
			RLtesteRecordList result = new RLtesteRecordList();
			result.InnerFromArray(array);
			return result;
		}

		public static RLtesteRecordList ToList<T>(T[] array, Func <T, RCtesteRecord> converter) {
			RLtesteRecordList result = new RLtesteRecordList();
			result.InnerFromArray(array, converter);
			return result;
		}

		public static RLtesteRecordList FromRestList<T>(RestList<T> restList, Func <T, RCtesteRecord> converter) {
			RLtesteRecordList result = new RLtesteRecordList();
			result.InnerFromRestList(restList, converter);
			return result;
		}
		...
		/// <param name="trans"> IDbTransaction Parameter</param>
		[Obsolete("Use the Default Constructor and set the Transaction afterwards.")]
		public RLtesteRecordList(IDbTransaction trans): base(trans) {
		}
		...
		/// <param name="alternateReadDBMethod"> Alternate Read Method</param>
		[Obsolete("Use the Default Constructor and set the Transaction afterwards.")]
		public RLtesteRecordList(IDbTransaction trans, ReadDBMethodDelegate alternateReadDBMethod): this(trans) {
			this.alternateReadDBMethod = alternateReadDBMethod;
		}
		...
		/// <param name="info"> SerializationInfo</param>
		/// <param name="context"> StreamingContext</param>
		public RLtesteRecordList(SerializationInfo info, StreamingContext context): base(info, context) {
		}
		...
		/// </summary>
		/// <returns>The new record list</returns>
		protected override OSList<RCtesteRecord> NewList() {
			return new RLtesteRecordList();
		}
